    ...
    for (const payload of payloads) {
      ...
      for (const tree of TREE_IDS) {
        for (const season of ['spring', 'summer', 'autumn'] as const) {
          if (scanTreePayload(grid, colorsOf(season, tree)) !== payload) failures.push(`v${grid.version} ${tree} ${season}`)
        }
      }
    }
