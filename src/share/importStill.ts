...
export const STILL_ERROR = 'This image is not an ictree share.'
...
export async function readStillFile(file: File): Promise<ShareState> {
  ...
  const chunk = readPngChunk(buf, PNG_CHUNK_TYPE)
  candidates.push(chunk ? unframeStillPayload(chunk) : null)
