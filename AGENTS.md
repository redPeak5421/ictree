...
- Species follows the palette. There is no tree-variety picker.
...
6. `buildShareSearch` writes `?u=&s=&p=` plus `e=1` when locked and `m=r` in Reveal. That search string is what still/loop metadata stores.
...
| `docs/`, `.omc/plans/` | Historical specs. Prefer source over these when they disagree |
...
**Determinism.** `hashString(payload)` seeds layout only. Same payload + palette + season → same tree. Species is **not** hashed from the URL.

**Palette → species / ornaments / cover** (`palettes.ts`, `treeSpecies.ts`):
...
| `src/share/params.ts` | `ShareState = { url, season, palette, locked, mode }` |
...
| `src/scene/tree.ts` / `treeSpecies.ts` / `palettes.ts` | Crown + palette contracts |
...
`src/scene/moundGeometry.ts` is leftover and unused.
...
- Share params: `u/s/p` plus `e`/`m`; ignore leftover `t=`.
...
- Species from palette; crown layers populated; finder blades stay in dark ink.
...
- `VarietyBar`, `t=` variety query, or `speciesForPayload` as a species resolver
- Encoding `buildShareSearch` into the living grove QR
- A second tree-species picker
- Docs the user did not ask for

`applyShareState` must: set Reveal, apply season/palette, reset camera to `VIEW_PITCH` / `VIEW_YAW`, set `url` to `DEFAULT_PAYLOAD` when locked (otherwise the imported URL). Keep `preserveDrawingBuffer` and `[data-grove-canvas]`.
